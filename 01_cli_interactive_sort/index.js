...
const getUserInput = () => {
    cli.question('Enter a few words or numbers separated by a space (or type "exit" to quit): ', (input) => {
        if (input.toLowerCase() === 'exit') {
            cli.close();
        } else {
            values = processInput(input); // Оновлюємо значення
            showOperationMenu();
        }
    });
}
...
const showOperationMenu = () => {
    cli.question(`Choose an operation:
...
Enter the number corresponding to your choice: `, (choice) => {
        switch (parseInt(choice)) {
            ...
            case 4:
                ...
                break;
            case 5:
                const uniqueWords = values.filter((value, index, self) => self.indexOf(value) === index && typeof value === 'string');
